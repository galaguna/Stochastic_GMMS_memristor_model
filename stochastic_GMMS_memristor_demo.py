...
# and switching thresholds (V_ON and V_OFF) variability with
# subjacent alpha-stable random variables.
#******************************************************************
...
if __name__ == '__main__':
    print("Hello, simulating Knowm memristor with alpha-stable stochastic GMMS")
    ...
    dt2Tao = dt / Tao
    epsilon = q /(k * T) #Variable of MMS model
    ...
    for n in range(N):
        ...
        for i in range(1, size):
            ...
            X[i] = dt2Tao * ((1- X[i-1]) * (1/(1 + np.exp(-epsilon *(Vm[i]-Von)))) - X[i-1] * (1-(1/(1 + np.exp(-epsilon *(Vm[i]-Voff)))))) + X[i-1]
            G[i] = (X[i]/Ron)+((1-X[i])/Roff)

            #Schottky diode effect adjustment (GMMS model):
            Is = A_f * np.exp(B_f * Vm[i]) - A_r * np.exp(B_r * Vm[i]) #Schottky diode current
